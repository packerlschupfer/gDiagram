namespace GDiagram {
    public class MermaidClassRenderer : Object {
        ...
        public string generate_dot(MermaidClassDiagram diagram) {
            ...
            dot.append("  node [fontname=\"Sans\", fontsize=10, shape=record, style=\"filled\", fillcolor=\"%s\", fontcolor=\"%s\", color=\"%s\"];\n".printf(palette.node_fill, palette.node_text, palette.node_border));
        ...
        private void render_class(StringBuilder dot, MermaidClass cls) {
            ...
            // Stereotype annotation (<<interface>>, <<abstract>>, etc.)
            string stereotype_text = get_stereotype_label(cls);
            if (stereotype_text.length > 0) {
                label.append_printf("%s\\n", stereotype_text);
            }

            // Class name (record label context — escape <, >, {, })
            string class_name = RenderUtils.escape_record_label(cls.name);
            ...
            if (cls.members.size > 0) {
                ...
                foreach (var member in cls.members) {
                    ...
                    // Member name
                    string member_name = RenderUtils.escape_record_label(member.name);
                    label.append(member_name);

                    // Method parentheses (before return type in UML notation)
                    if (member.is_method) {
                        label.append("()");
                    }

                    // Type annotation (return type for methods, field type for attributes)
                    if (member.type_name != null && member.type_name.length > 0) {
                        string type_str = RenderUtils.escape_record_label(member.type_name);
        ...
        private string get_stereotype_label(MermaidClass cls) {
            // In Graphviz record labels, < and > must be escaped as \< and \>
            ...
            return "\\<\\<%s\\>\\>".printf(RenderUtils.escape_record_label(st));
        }
        ...
        private string get_relation_arrowhead(MermaidRelationType type) {
            switch (type) {
                ...
                case MermaidRelationType.AGGREGATION:
                    // Diamond is on the tail (source/owner end), plain line at head
                    return "none";
                case MermaidRelationType.DEPENDENCY:
