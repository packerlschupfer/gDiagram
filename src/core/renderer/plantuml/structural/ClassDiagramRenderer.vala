namespace GDiagram {
    public class ClassDiagramRenderer : Object {
        ...
        public string generate_dot(ClassDiagram diagram) {
            ...
            string class_bg = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("class", "BackgroundColor") ?? palette.node_fill);
            ...
            string class_font_color = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("class", "FontColor") ?? palette.node_text);
            ...
            string edge_color = palette.edge_color;
            string edge_text_color = palette.edge_text;
            ...
            foreach (var c in diagram.classes) {
                ...
                if (c.color != null && c.color.length > 0) {
                    string safe_color = RenderUtils.sanitize_color(c.color);
                    sb.append("  %s [label=\"%s\", fillcolor=\"%s\", fontcolor=\"%s\"];\n".printf(
                        id, label, safe_color, RenderUtils.contrast_text(safe_color)));
                } else {
                    sb.append("  %s [label=\"%s\"];\n".printf(id, label));
                }
            }
            ...
            if (diagram.notes.size > 0) {
                sb.append("  // Notes\n");
                string note_bg = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("note", "BackgroundColor") ?? palette.accent_secondary);
                string note_border = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("note", "BorderColor") ?? palette.node_border);
                string note_font = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("note", "FontColor") ?? RenderUtils.contrast_text(note_bg));

                foreach (var note in diagram.notes) {
                    string note_label = RenderUtils.escape_label(note.text);
                    sb.append("  %s [shape=note, label=\"%s\", fillcolor=\"%s\", color=\"%s\", fontcolor=\"%s\"];\n".printf(
                        note.id, note_label, note_bg, note_border, note_font
                    ));
            ...
            foreach (var rel in diagram.relationships) {
                ...
                sb.append("  %s -> %s [style=%s, arrowhead=%s, arrowtail=%s, dir=%s".printf(
                    from_id, to_id, style, arrowhead, arrowtail, dir
                ));
                if (label != "") {
                    // ortho splines don't support label — use xlabel instead
                    sb.append(", xlabel=\"%s\"".printf(label));
                }
                sb.append("];\n");
            }
        ...
        private string build_class_label(UmlClass c) {
            ...
            if (c.stereotype != null && c.stereotype.length > 0) {
                // Custom stereotype
                stereotype = "\\<\\<%s\\>\\>\\n".printf(RenderUtils.escape_record_label(c.stereotype));
            } else {
            ...
            sb.append(RenderUtils.escape_record_label(label_name));
            ...
            if (c.members.size > 0) {
                ...
                foreach (var f in fields) {
                    ...
                    sb.append(" ");
                    sb.append(RenderUtils.escape_record_label(f.name));
                    sb.append("\\l");
                }
                ...
                foreach (var m in methods) {
                    ...
                    sb.append(" ");
                    sb.append(RenderUtils.escape_record_label(m.name));
                    sb.append("\\l");
                }
            }
