    ...
    public class UseCaseDiagramRenderer : Object {
        ...
        public string generate_dot(UseCaseDiagram diagram) {
            ...
            foreach (var actor in diagram.actors) {
                ...
                string fill = actor.color != null ? RenderUtils.sanitize_color(actor.color) : actor_color;
                // Use a simple shape for actors - stick figure would require custom SVG
                sb.append("  %s [label=\"%s\", shape=ellipse, style=filled, fillcolor=\"%s\", color=\"%s\", fontcolor=\"%s\"];\n".printf(
                    id, RenderUtils.escape_label(label), fill, actor_border, RenderUtils.contrast_text(fill)));
            }

            // Render use cases
            string uc_color = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("usecase", "BackgroundColor") ?? palette.component_fill);
            ...
            foreach (var uc in diagram.use_cases) {
                ...
                string fill = uc.color != null ? RenderUtils.sanitize_color(uc.color) : uc_color;
                sb.append("  %s [label=\"%s\", shape=ellipse, style=filled, fillcolor=\"%s\", color=\"%s\", fontcolor=\"%s\"];\n".printf(
                    id, RenderUtils.escape_label(label), fill, uc_border, RenderUtils.contrast_text(fill)));
            }
            ...
            string rect_color = RenderUtils.sanitize_color(diagram.skin_params.get_element_property("rectangle", "BackgroundColor") ?? palette.node_fill);
        ...
        public uint8[]? render_to_svg(UseCaseDiagram diagram) {
            ...
            return RenderUtils.run_graphviz_subprocess(dot, layout_engine, "usecase");
        }
