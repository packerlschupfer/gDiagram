...
public class ArchimateDiagramRenderer : Object {
    ...
    // PlantUML's ArchiMate layer colours (its "#Business", "#Application", ...
    // colour names): light fills that keep dark text on any theme
    public static string layer_fill(ArchimateLayer layer) {
        switch (layer) {
            case ArchimateLayer.BUSINESS:       return "#FFFFCC";
            case ArchimateLayer.APPLICATION:    return "#C2F0FF";
            case ArchimateLayer.TECHNOLOGY:     return "#C9FFC9";
            case ArchimateLayer.MOTIVATION:     return "#CCCCFF";
            case ArchimateLayer.PHYSICAL:       return "#97FF97";
            case ArchimateLayer.IMPLEMENTATION: return "#FFE0E0";
            case ArchimateLayer.STRATEGY:       return "#F8E7C0";
            default:                            return ThemeManager.get_active_palette().node_fill;
        }
    }
    ...
    private string get_layer_color(ArchimateLayer layer, string? override_color) {
        ...
        return layer_fill(layer);
    }

    // A thin dark outline, as PlantUML draws its ArchiMate elements
    private string get_layer_border(ArchimateLayer layer) {
        if (layer == ArchimateLayer.NONE) {
            return ThemeManager.get_active_palette().node_border;
        }
        return "#555555";
    }
