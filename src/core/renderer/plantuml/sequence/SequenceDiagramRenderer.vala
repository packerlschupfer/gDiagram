    ...
    public class SequenceDiagramRenderer : Object {
        ...
        public string generate_dot(SequenceDiagram diagram) {
            ...
            foreach (var p in diagram.participants) {
                ...
                string color = normalize_color(p.color) ?? palette.container_fill;
            ...
            sb.append("  // Lifelines\n");
            sb.append("  edge [style=dashed, arrowhead=none, color=\"%s\", penwidth=1.5, weight=100];\n".printf(palette.grid));
            foreach (var p in diagram.participants) {
            ...
            if (diagram.notes.size > 0) {
                ...
                foreach (var note in diagram.notes) {
                    ...
                    sb.append("  %s [shape=note, style=filled, fillcolor=\"%s\", fontcolor=\"%s\", label=\"%s\"];\n".printf(
                        note_id, palette.accent_secondary,
                        RenderUtils.contrast_text(palette.accent_secondary), note_text
                    ));
        ...
        private void render_frame_cluster(StringBuilder sb, SequenceFrame frame, SequenceDiagram diagram) {
            ...
            sb.append("    color=\"%s\";\n".printf(ThemeManager.get_active_palette().boundary_stroke));
            sb.append("    fontname=\"Sans Bold\";\n");
            sb.append("    fontsize=10;\n");

            // Find messages that belong to this frame and include their actual node IDs
            int msg_idx = 0;
            var included_nodes = new Gee.HashSet<string>();
            foreach (var msg in diagram.messages) {
                bool in_frame = is_message_in_frame(diagram, msg, frame);
                if (in_frame) {
                    string from_id = RenderUtils.escape_id(msg.from.get_id()) + "_m%d".printf(msg_idx);
                    string to_id = RenderUtils.escape_id(msg.to.get_id()) + "_m%d".printf(msg_idx);
                    included_nodes.add(from_id);
                    included_nodes.add(to_id);
                }
                msg_idx++;
            }
            foreach (var node_id in included_nodes) {
                sb.append("    %s;\n".printf(node_id));
            }
        ...
        // Frame fills use semantic palette slots — ALT/CRITICAL highlight
        // with accent_secondary, BREAK uses warning, LOOP uses success, etc.
