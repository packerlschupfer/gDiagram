namespace GDiagram {
    public enum DiagramFormat {
        PLANTUML,
        MERMAID,
        UNKNOWN
    }

    public enum DiagramType {
        SEQUENCE,
        CLASS,
        USECASE,
        ACTIVITY,
        STATE,
        COMPONENT,
        OBJECT,
        ER_DIAGRAM,
        MINDMAP,
        WBS,
        GANTT,
        JSON_DIAGRAM,
        YAML_DIAGRAM,
        CHRONOLOGY,
        TIMING,
        NWDIAG,
        ARCHIMATE,
        // Mermaid-specific types
        MERMAID_FLOWCHART,
        MERMAID_SEQUENCE,
        MERMAID_STATE,
        MERMAID_CLASS,
        MERMAID_ER,
        MERMAID_GANTT,
        MERMAID_PIE,
        MERMAID_USER_JOURNEY,
        MERMAID_GIT_GRAPH,
        MERMAID_MINDMAP,
        MERMAID_TIMELINE,
        MERMAID_QUADRANT,
        MERMAID_XYCHART,
        MERMAID_KANBAN,
        MERMAID_SANKEY,
        MERMAID_REQUIREMENT,
        MERMAID_BLOCK,
        MERMAID_PACKET,
        MERMAID_C4,
        MERMAID_ARCHITECTURE,
        MERMAID_ZENUML,
        MERMAID_RADAR,
        MERMAID_TREEMAP,
        // Additional PlantUML types
        DOT_DIAGRAM,       // @startdot — raw Graphviz DOT passthrough
        SALT,              // @startsalt — UI wireframe/mockup
        CHEN_ER,           // @startchen — Chen ER notation
        EBNF,              // @startebnf — EBNF grammar railroad diagrams
        REGEX_DIAGRAM,     // @startregex — regular expression visualization
        TREE,              // @starttree — tree structure
        DITAA,             // @startditaa — ASCII art diagrams
        BOARD,             // @startboard — board/kanban layout
        ANCESTRY,          // @startancestry — family tree / ancestry diagram
        UNKNOWN
    }

    public enum ParticipantType {
        PARTICIPANT,
        ACTOR,
        BOUNDARY,
        CONTROL,
        ENTITY,
        DATABASE,
        COLLECTIONS,
        QUEUE
    }

    public enum ArrowStyle {
        SOLID,          // ->
        DOTTED,         // -->
        SOLID_OPEN,     // ->>
        DOTTED_OPEN     // -->>
    }

    public enum ArrowDirection {
        RIGHT,
        LEFT,
        BIDIRECTIONAL
    }

    public class Participant : Object {
        public string name { get; set; }
        public string? alias { get; set; }
        public string? display_label { get; set; }  // Multi-line label for rendering
        public ParticipantType participant_type { get; set; }
        public string? color { get; set; }
        public string? stereotype { get; set; }  // "foo" from "participant Bob <<foo>>"
        public int source_line { get; set; }
        // "create Bob": the head box is drawn at the first message to Bob, not at the top
        public bool created { get; set; default = false; }
        // "participant X order 10": columns are sorted by this (stable, default 0)
        public int order { get; set; default = 0; }
        // "<< (C,#ADD1B2) Testable >>": the spot letter and its circle colour
        public string? spot_char { get; set; default = null; }
        public string? spot_color { get; set; default = null; }
        // "participant P [ =Title / ---- / ""Sub"" ]": the raw body lines, "----" a separator
        public Gee.ArrayList<string>? body_lines { get; set; default = null; }

        public Participant(string name, ParticipantType type = ParticipantType.PARTICIPANT, int line = 0) {
            this.name = name;
            this.participant_type = type;
            this.alias = null;
            this.color = null;
            this.source_line = line;
        }

        public string get_id() {
            return alias ?? name;
        }
    }

    // The diagram edge a border message ("[-> A", "A ->]", "?-> A", "A ->?") comes from
    // or goes to: LEFT when the free end is written before the arrow, RIGHT after it.
    public enum MessageBorder {
        NONE,
        LEFT,
        RIGHT
    }

    public class Message : Object {
        public Participant from { get; set; }
        public Participant to { get; set; }
        public string? label { get; set; }
        public ArrowStyle style { get; set; }
        public ArrowDirection direction { get; set; }
        public bool activate_target { get; set; }      // ++ at end
        public bool deactivate_source { get; set; }    // -- at end
        public int number { get; set; }                // autonumber value, -1 when off
        public string? color { get; set; }             // "-[#red]>"; null for the theme's
        // "[-> A" / "A ->]": one end is the diagram border instead of a participant; from
        // and to are then both the participant. "?-> A" / "A ->?" are short arrows that
        // stop just beside the lifeline (border_short).
        public MessageBorder border { get; set; }
        public bool border_short { get; set; }
        // The arrow ends as written: head_left "<", "<<", "/", "//", "\", "\\" or "";
        // head_right ">", ">>", "\", "\\", "/", "//" or ""; deco_left / deco_right "o", "x" or ""
        public string head_left { get; set; default = ""; }
        public string head_right { get; set; default = ""; }
        public string deco_left { get; set; default = ""; }
        public string deco_right { get; set; default = ""; }
        // The autonumber as drawn, format applied ("<b>[005]"); null when off
        public string? number_text { get; set; default = null; }
        // "A -> B ++ #gold": the colour of the activation the message starts
        public string? activation_color { get; set; default = null; }
        // "{start} A -> B" (teoz anchor)
        public string? anchor { get; set; default = null; }

        public Message(Participant from, Participant to) {
            this.from = from;
            this.to = to;
            this.label = null;
            this.style = ArrowStyle.SOLID;
            this.direction = ArrowDirection.RIGHT;
            this.activate_target = false;
            this.deactivate_source = false;
            this.number = -1;
            this.border = MessageBorder.NONE;
            this.border_short = false;
        }
    }

    public enum ActivationType {
        ACTIVATE,
        DEACTIVATE,
        DESTROY
    }

    public class Activation : Object {
        public Participant participant { get; set; }
        public ActivationType activation_type { get; set; }
        // "activate A #red": the bar's fill; null for the theme's
        public string? color { get; set; default = null; }

        public Activation(Participant participant, ActivationType type) {
            this.participant = participant;
            this.activation_type = type;
        }
    }

    public class Note : Object {
        public string text { get; set; }
        public string position { get; set; }  // "left", "right", "over"
        public Participant? participant { get; set; }
        // Second participant of "note over A, B"
        public Participant? participant2 { get; set; }
        // "note" (folded corner), "hnote" (hexagon) or "rnote" (rectangle)
        public string kind { get; set; }
        // "note over A #lightblue : x"; null for the theme's note colour
        public string? color { get; set; }
        // "/ note over B": drawn on the row of the note before it
        public bool aligned { get; set; default = false; }

        public Note(string text, string position) {
            this.text = text;
            this.position = position;
            this.participant = null;
            this.kind = "note";
            this.color = null;
        }
    }

    public class ParseError : Object {
        public string message { get; set; }
        public int line { get; set; }
        public int column { get; set; }

        public ParseError(string message, int line, int column) {
            this.message = message;
            this.line = line;
            this.column = column;
        }

        public string to_string() {
            return "Line %d:%d: %s".printf(line, column, message);
        }
    }

    // Base class for sequence events (for ordering)
    public abstract class SequenceEvent : Object {
        public int order { get; set; }
    }

    public class MessageEvent : SequenceEvent {
        public Message message { get; set; }
        public MessageEvent(Message msg, int order) {
            this.message = msg;
            this.order = order;
        }
    }

    public class NoteEvent : SequenceEvent {
        public Note note { get; set; }
        public NoteEvent(Note note, int order) {
            this.note = note;
            this.order = order;
        }
    }

    public class ActivationEvent : SequenceEvent {
        public Activation activation { get; set; }
        public ActivationEvent(Activation act, int order) {
            this.activation = act;
            this.order = order;
        }
    }

    // Grouping frames (alt/opt/loop/par/break/critical/group)
    public enum SequenceFrameType {
        ALT,        // alt/else/end
        OPT,        // opt/end (optional)
        LOOP,       // loop/end
        PAR,        // par/end (parallel)
        BREAK,      // break/end
        CRITICAL,   // critical/end
        GROUP,      // group/end (generic)
        REF,        // ref (reference to another diagram)
        ELSE        // else section within alt
    }

    public class SequenceFrame : Object {
        public string id { get; set; }
        public SequenceFrameType frame_type { get; set; }
        public string? label { get; set; }
        public string? condition { get; set; }
        public int start_order { get; set; }
        public int end_order { get; set; }
        public int source_line { get; set; }
        public Gee.ArrayList<SequenceFrame> sections { get; private set; }  // For else sections in alt
        public SequenceFrame? parent { get; set; }
        // "ref over A, B": the participants a reference box spans
        public Gee.ArrayList<Participant> participants { get; private set; }

        private static int frame_counter = 0;

        public SequenceFrame(SequenceFrameType type, int line = 0) {
            this.id = "_frame_%d".printf(frame_counter++);
            this.frame_type = type;
            this.label = null;
            this.condition = null;
            this.start_order = 0;
            this.end_order = 0;
            this.source_line = line;
            this.sections = new Gee.ArrayList<SequenceFrame>();
            this.parent = null;
            this.participants = new Gee.ArrayList<Participant>();
        }

        public static void reset_counter() {
            frame_counter = 0;
        }

        public string get_type_label() {
            switch (frame_type) {
                case SequenceFrameType.ALT:
                    return "alt";
                case SequenceFrameType.OPT:
                    return "opt";
                case SequenceFrameType.LOOP:
                    return "loop";
                case SequenceFrameType.PAR:
                    return "par";
                case SequenceFrameType.BREAK:
                    return "break";
                case SequenceFrameType.CRITICAL:
                    return "critical";
                case SequenceFrameType.GROUP:
                    return "group";
                case SequenceFrameType.REF:
                    return "ref";
                case SequenceFrameType.ELSE:
                    return "else";
                default:
                    return "group";
            }
        }
    }

    public class FrameEvent : SequenceEvent {
        public SequenceFrame frame { get; set; }
        public bool is_start { get; set; }
        public FrameEvent(SequenceFrame frame, bool is_start, int order) {
            this.frame = frame;
            this.is_start = is_start;
            this.order = order;
        }
    }

    // "== Title ==" divider across all lifelines
    public class SequenceDivider : Object {
        public string title { get; set; }
        public int source_line { get; set; }

        public SequenceDivider(string title, int line) {
            this.title = title;
            this.source_line = line;
        }
    }

    public class DividerEvent : SequenceEvent {
        public SequenceDivider divider { get; set; }
        public DividerEvent(SequenceDivider divider, int order) {
            this.divider = divider;
            this.order = order;
        }
    }

    // "..." / "...5 minutes later..." (a delay: dotted lifelines) and "|||" / "||45||"
    // (extra vertical space)
    public class SequenceSpace : Object {
        public bool delay { get; set; }
        public string? text { get; set; }
        public int height { get; set; }

        public SequenceSpace(bool delay, string? text, int height) {
            this.delay = delay;
            this.text = text;
            this.height = height;
        }
    }

    public class SpaceEvent : SequenceEvent {
        public SequenceSpace space { get; set; }
        public SpaceEvent(SequenceSpace space, int order) {
            this.space = space;
            this.order = order;
        }
    }

    // "newpage [title]": the diagram continues on a new page
    public class PageBreakEvent : SequenceEvent {
        public string? title { get; set; }
        public PageBreakEvent(string? title, int order) {
            this.title = title;
            this.order = order;
        }
    }

    // "{start} <-> {end} : label" (teoz): a vertical arrow between two anchored messages
    public class SequenceDuration : Object {
        public string from_anchor { get; set; }
        public string to_anchor { get; set; }
        public string? label { get; set; }

        public SequenceDuration(string from_anchor, string to_anchor, string? label) {
            this.from_anchor = from_anchor;
            this.to_anchor = to_anchor;
            this.label = label;
        }
    }

    public class SequenceDiagram : Object {
        public DiagramType diagram_type { get; private set; }
        public Gee.ArrayList<Participant> participants { get; private set; }
        public Gee.ArrayList<Message> messages { get; private set; }
        public Gee.ArrayList<Note> notes { get; private set; }
        public Gee.ArrayList<Activation> activations { get; private set; }
        public Gee.ArrayList<SequenceFrame> frames { get; private set; }
        public Gee.ArrayList<SequenceDivider> dividers { get; private set; }
        public Gee.ArrayList<SequenceEvent> events { get; private set; }
        public Gee.ArrayList<ParseError> errors { get; private set; }
        public string? title { get; set; }
        public string? header { get; set; }
        public string? footer { get; set; }
        public string? caption { get; set; }
        // skinparam settings; the renderer uses them for theme colours
        public SkinParams skin_params { get; private set; }
        // "hide footbox": no participant boxes at the bottom
        public bool hide_footbox { get; set; default = false; }
        public Gee.ArrayList<SequenceDuration> durations { get; private set; }
        // 1 + the "newpage" breaks (none are recorded under "ignore newpage")
        public int page_count { get; set; default = 1; }
        private int event_counter = 0;

        public SequenceDiagram() {
            this.diagram_type = DiagramType.SEQUENCE;
            this.participants = new Gee.ArrayList<Participant>();
            this.messages = new Gee.ArrayList<Message>();
            this.notes = new Gee.ArrayList<Note>();
            this.activations = new Gee.ArrayList<Activation>();
            this.frames = new Gee.ArrayList<SequenceFrame>();
            this.dividers = new Gee.ArrayList<SequenceDivider>();
            this.events = new Gee.ArrayList<SequenceEvent>();
            this.errors = new Gee.ArrayList<ParseError>();
            this.title = null;
            this.header = null;
            this.footer = null;
            this.skin_params = new SkinParams();
            this.durations = new Gee.ArrayList<SequenceDuration>();

            // Reset counters
            SequenceFrame.reset_counter();
        }

        public void add_message(Message msg) {
            messages.add(msg);
            events.add(new MessageEvent(msg, event_counter++));
        }

        public void add_note(Note note) {
            notes.add(note);
            events.add(new NoteEvent(note, event_counter++));
        }

        public void add_divider(SequenceDivider divider) {
            dividers.add(divider);
            events.add(new DividerEvent(divider, event_counter++));
        }

        public void add_space(SequenceSpace space) {
            events.add(new SpaceEvent(space, event_counter++));
        }

        public void add_page_break(string? title) {
            page_count++;
            events.add(new PageBreakEvent(title, event_counter++));
        }

        public void add_activation(Activation act) {
            activations.add(act);
            events.add(new ActivationEvent(act, event_counter++));
        }

        public bool has_errors() {
            return errors.size > 0;
        }

        public Participant? find_participant(string name) {
            foreach (var p in participants) {
                if (p.name == name || p.alias == name) {
                    return p;
                }
            }
            return null;
        }

        public Participant get_or_create_participant(string name) {
            var existing = find_participant(name);
            if (existing != null) {
                return existing;
            }

            var participant = new Participant(name);
            participants.add(participant);

            // Debug: log phantom participant creation
            bool debug = Environment.get_variable("G_MESSAGES_DEBUG") != null;
            if (debug && participants.size > 10) {
                printerr("[WARNING] Creating participant #%d: '%s'\n", participants.size, name);
            }

            return participant;
        }
    }
}
