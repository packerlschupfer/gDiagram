namespace GDiagram {
    ...
    public class SkinParams : Object {
        ...
        public string? get_element_property(string element, string property) {
            ...
            // <style> root / element properties reach every element
            if (element_params.contains("element")) {
                var generic = element_params.get("element");
                if (generic.contains(prop_key)) {
                    return generic.get(prop_key);
                }
            }
            if (global_params.contains("element" + prop_key)) {
                return global_params.get("element" + prop_key);
            }

            // Fall back to global — except BackgroundColor. A bare
            // "skinparam backgroundColor" is the canvas, not every element's
            // fill: falling back painted notes and states in the canvas colour,
            // which on a dark theme made them invisible.
            if (prop_key == "backgroundcolor") {
                return null;
            }
        ...
        /**
         * A property set for one stereotype: "skinparam class { BackgroundColor<<entity>> ... }"
         * (what a <style> ".entity" selector translates to). Looks at the element, then
         * at "element" for stereotype selectors that name no element. Null otherwise.
         */
        public string? get_stereotype_property(string element, string property, string? stereotype) {
            if (stereotype == null || stereotype.strip().length == 0) {
                return null;
            }
            string key = "%s<<%s>>".printf(property, stereotype.strip()).down();
            foreach (string elem in new string[] { element.down(), "element" }) {
                if (element_params.contains(elem)) {
                    var props = element_params.get(elem);
