    ...
    private void parse_nwdiag(string source) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (lower.has_prefix("network ")) {
                ...
                current_network = new NwNetwork(net_name, line_nos[i]);
            ...
            if (lower.has_prefix("group")) {
                ...
                current_group = new NwGroup(grp_name, line_nos[i]);
            ...
            if (trimmed.contains(" -- ") && current_network == null && current_group == null) {
                ...
                if (parts.length == 2) {
                    diagram.peer_links.add(new NwPeerLink(
                        parts[0].strip(), parts[1].strip().replace(";", ""), line_nos[i]
                    ));
                }
                continue;
            }
            ...
            if (current_group != null && current_network == null) {
                // "web01" or "web01, web02;"
                foreach (string part in trimmed.replace(";", "").split(",")) {
                    string node_name = part.strip();
            ...
            if (current_network != null) {
                parse_network_node(trimmed, current_network, line_nos[i]);
            }
        }
    }
