namespace GDiagram {
    public class StateDiagramParser : Object {
        ...
        private void parse_state_declaration() throws Error {
            ...
            // Check for color. The lexer's scan_color() returns "#RRGGBB" or
            // "#name" as a single IDENTIFIER, so a bare HASH check misses the
            // common form entirely.
            if (check(TokenType.IDENTIFIER) && peek().lexeme.has_prefix("#")) {
                state.color = advance().lexeme;
        ...
        private void parse_initial_final_transition() {
            advance();  // consume [*]
        ...
        private void parse_history_transition() {
        ...
        private void parse_transition_or_state() {
            string from_name;
            if (check(TokenType.STRING)) {
        ...
        private void parse_nested_initial_final(State parent) {
            advance();  // consume [*]
        ...
        private void parse_nested_transition(State parent) {
            string from_name;
            if (check(TokenType.STRING)) {
                from_name = advance().lexeme;
            } else {
