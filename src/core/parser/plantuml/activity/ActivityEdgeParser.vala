namespace GDiagram {
    ...
    public class ActivityEdgeParser : Object {
        ...
        public ActivityEdgeParser(TokenStream stream, ActivityDiagram diagram) {
            this.stream = stream;
         ...
         */
        public void parse_arrow_label() {
            string label = collect_label();
            if (label.length > 0) {
                pending_edge_label = label;
            }
        }

        // Arrow label text up to the ";" that ends a line. Like PlantUML, a label
        // without one continues on the next lines ("-> line one\nline two;"): stopping
        // at the line end turned the following lines into a bogus action node. An
        // arrow alone on its line ("->", "-[#red]->") has no label.
        private string collect_label() {
            ...
            while (!is_at_end() && !check(TokenType.ENDUML)) {
                ...
                if (t.token_type == TokenType.SEMICOLON) {
                    if (check(TokenType.NEWLINE) || check(TokenType.COMMENT) || is_at_end()) {
                        break;
                    }
                    sb.append(";");
                    continue;
         ...
         */
        public void parse_styled_arrow() {
            ...
            // Optional label after the arrow, up to the ";" ending a line
            string label = collect_label();
            if (label.length > 0) {
                pending_edge_label = label;
            }
