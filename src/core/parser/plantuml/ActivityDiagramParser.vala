namespace GDiagram {
    ...
    public class ActivityDiagramParser : Object {
        ...
        // Branch label and no-branch flag of a pending connection ("else (yes)" with an
        // empty body, "endwhile (no)"): the edge is only drawn once the next node comes
        private Gee.HashMap<ActivityNode, string> pending_labels;
        private Gee.HashSet<ActivityNode> pending_no_branch;
        ...
        private Gee.ArrayList<ActivityNode> rank_after_terminals;

        // The element a note written at this point belongs to, as in PlantUML: the last
        // node drawn, also after a stop or a block that ends without a node to continue
        // from (last_node is null there, and such notes were drawn unattached at the top).
        // After an if it is the condition, after a fork its join; after a while, repeat,
        // switch, group or partition the last element inside. Merge diamonds and invisible
        // pass-through points are skipped.
        private ActivityNode? note_anchor;
        private ActivityNode? previous_note_anchor;
        // Notes before the first node: placed above it once it comes
        private Gee.ArrayList<ActivityNote> leading_notes;
        ...
        public ActivityDiagramParser() {
            ...
            this.pending_labels = new Gee.HashMap<ActivityNode, string>();
            this.pending_no_branch = new Gee.HashSet<ActivityNode>();
            ...
            this.rank_after_terminals = new Gee.ArrayList<ActivityNode>();
            this.leading_notes = new Gee.ArrayList<ActivityNote>();
        }
        ...
        public ActivityDiagram parse(Gee.ArrayList<Token> tokens) {
            ...
            this.pending_no_branch.clear();
            this.current_partition = null;
            this.connectors.clear();
            this.pending_breaks.clear();
            this.rank_after_terminals.clear();
            this.note_anchor = null;
            this.previous_note_anchor = null;
            this.leading_notes.clear();
            ...
            control_flow_parser.rank_after_terminals = rank_after_terminals;
            control_flow_parser.branch_started.connect(on_branch_started);
            ...
            control_flow_parser.set_callbacks(
                ...
                (node) => { last_node = node; set_note_anchor(node); },
                // An empty "else (yes)" adds its condition twice: once for the
                // else branch and once for the fall-through, which drew two edges
                (node, label, no_branch) => {
                    if (!pending_connections.contains(node)) pending_connections.add(node);
                    if (label != null && label.length > 0) pending_labels.set(node, label);
                    if (no_branch) pending_no_branch.add(node);
                },
                () => { return last_node; },
                (node) => {
                    pending_connections.remove(node);
                    pending_labels.unset(node);
                    pending_no_branch.remove(node);
                },
                () => { flush_pending(); }
            );
        ...
        private void parse_statement_impl() throws Error {
            ...
            if (match(TokenType.BREAK)) {
                // PlantUML draws no symbol: the flow leaves for the loop exit. The point
                // is invisible and keeps the branch label of a break right after "then".
                var break_node = new ActivityNode(ActivityNodeType.MERGE, null, previous().line);
                break_node.passthrough = true;
            ...
            // Coloured action "#pink:text;" (one IDENTIFIER "#pink", or HASH + name)
            if (ActivityParserUtils.is_color_token(peek()) || match(TokenType.HASH)) {
                // "#pink:if (...)" colours the condition. "#pink:while (...)" is a plain
                // while: PlantUML 1.2026.1 parses it but draws no colour.
                // Only with "(" after the keyword: "#pink:if needed, retry;" is an action.
                if (ActivityParserUtils.is_color_token(peek()) && check_next(TokenType.COLON)) {
                    TokenType keyword = stream.peek_at(stream.current + 2).token_type;
                    bool paren = stream.current + 3 < stream.size() &&
                        stream.peek_at(stream.current + 3).token_type == TokenType.LPAREN;
                    if (paren && (keyword == TokenType.IF || keyword == TokenType.WHILE)) {
                        string color = advance().lexeme;
                        advance();  // consume :
                        if (keyword == TokenType.IF) {
                            control_flow_parser.prefix_color = color;
                        }
                        return;  // the keyword is the next statement
                    }
                }
            ...
            if (match(TokenType.IF)) {
                ...
                int if_start = diagram.nodes.size;
                control_flow_parser.parse_if(previous().line);
                last_node = control_flow_parser.get_last_node();
                // A note after "endif" sits beside the condition
                for (int i = if_start; i < diagram.nodes.size; i++) {
                    if (diagram.nodes[i].node_type == ActivityNodeType.CONDITION) {
                        note_anchor = diagram.nodes[i];
                        break;
                    }
                }
                return;
            ...
            if (check(TokenType.FORK) && !check_fork_again()) {
                ...
                last_node = control_flow_parser.get_last_node();
                if (last_node != null) {
                    note_anchor = last_node;  // beside the join bar
                }
                return;
            }
            ...
            if (match(TokenType.REPEAT)) {
                int repeat_start = diagram.nodes.size;
                control_flow_parser.parse_repeat(previous().line);
                last_node = control_flow_parser.get_last_node();
                // A note after "repeat while" belongs to the last element of the loop
                // body, not to the loop condition that closes it
                ActivityNode? loop_condition = null;
                for (int i = repeat_start; i < diagram.nodes.size; i++) {
                    if (diagram.nodes[i].node_type == ActivityNodeType.CONDITION) {
                        loop_condition = diagram.nodes[i];
                    }
                }
                if (loop_condition != null && note_anchor == loop_condition && previous_note_anchor != null) {
                    note_anchor = previous_note_anchor;
                }
                return;
            }
        ...
        private void set_note_anchor(ActivityNode? node) {
            if (node == null || node.passthrough || node.node_type == ActivityNodeType.MERGE ||
                node == note_anchor) {
                return;
            }
            previous_note_anchor = note_anchor;
            note_anchor = node;
        }

        private void on_branch_started(ActivityNode condition) {
            set_note_anchor(condition);
        }

        // A note belongs to the element before it; a "floating note" is placed beside that
        // element without a connector. A note before any node is placed above the first one.
        private void parse_note(bool floating) {
            ActivityNode? anchor = note_anchor ?? last_node;
            metadata_parser.set_last_node(anchor);
            int count = diagram.notes.size;
            metadata_parser.parse_note(false);
            if (diagram.notes.size == count) {
                return;  // "note on link"
            }
            var note = diagram.notes[diagram.notes.size - 1];
            note.floating = floating;
            if (anchor == null) {
                note.position = NotePosition.TOP;
                leading_notes.add(note);
            }
        }

        // Pending connections become an invisible pass-through point, the new last node
        private void flush_pending() {
            if (pending_connections.size == 0) {
                return;
            }
            var point = new ActivityNode(ActivityNodeType.MERGE, null, previous().line);
            point.passthrough = true;
            add_node_with_connection(point);
        }
        ...
        private void add_node_with_connection(ActivityNode node) {
            ...
            // An arrow written where the flow continues from pending connections (right
            // after "else", "endwhile") styles those edges: "else\n-[#green,dotted]->"
            // lost its style, and the unused style leaked to a later edge
            bool styled_pending = last_node == null && pending_connections.size > 0;
            foreach (var pending in pending_connections) {
                string? pending_label = pending_labels.get(pending);
                if (styled_pending && (pending_label == null || pending_label.length == 0)) {
                    pending_label = edge_parser.pending_edge_label;
                }
                var pending_edge = new ActivityEdge(pending, node, pending_label);
                pending_edge.is_no_branch = pending_no_branch.contains(pending);
                if (styled_pending) {
                    pending_edge.color = edge_parser.pending_edge_color;
                    pending_edge.style = edge_parser.pending_edge_style;
                    pending_edge.direction = edge_parser.pending_edge_direction;
                    pending_edge.note = edge_parser.pending_edge_note;
                }
                diagram.add_edge(pending_edge);
            }
            if (styled_pending) {
                edge_parser.reset_pending_edge_attributes();
            }
            ...
            pending_no_branch.clear();

            foreach (var terminal in rank_after_terminals) {
                var spacer = new ActivityEdge(terminal, node);
                spacer.style = "hidden";
                diagram.add_edge(spacer);
            }
            rank_after_terminals.clear();

            last_node = node;
            set_note_anchor(node);
            if (!node.passthrough) {
                foreach (var note in leading_notes) {
                    note.attached_to = node;
                }
                leading_notes.clear();
            }
        }
