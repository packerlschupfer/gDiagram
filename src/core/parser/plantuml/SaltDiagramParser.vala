        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (trimmed.has_prefix("{")) {
                ...
                if (rest.length > 0 && rest != "}") {
                    parse_row(rest, panel, line_nos[i]);
                }
                continue;
            }
            ...
            if (trimmed == "}") {
                if (panel_stack.size > 1) {
                    // The closed panel keeps its rows; the parent already references it
                    // through its PANEL element
            ...
            var current_panel = panel_stack.get(panel_stack.size - 1);
            parse_row(trimmed, current_panel, line_nos[i]);
        }
    }
