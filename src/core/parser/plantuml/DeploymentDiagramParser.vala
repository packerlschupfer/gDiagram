namespace GDiagram {
    public class DeploymentDiagramParser : Object {
        ...
        private void parse_node_declaration(DeploymentNodeType type) throws Error {
            ...
            // Color (Lexer returns '#Color' as a single IDENTIFIER token)
            if (check(TokenType.IDENTIFIER) && peek().lexeme.has_prefix("#")) {
                node.color = try_parse_color_value();
            }
        ...
        private void parse_container_body(DeploymentNode container) {
            skip_newlines();
            ...
            while (!check(TokenType.RBRACE) && !is_at_end()) {
                ...
                if (nested_type != null) {
                    ...
                    if (name.length > 0) {
                        ...
                        if (check(TokenType.IDENTIFIER) && peek().lexeme.has_prefix("#")) {
                            child.color = try_parse_color_value();
                        }
        ...
        private void parse_connection_or_element() {
            string from_name;
            int from_line;
        ...
        // Handles IDENTIFIER tokens starting with '#' (Lexer returns '#Color' as one token)
        private string try_parse_color_value() {
            string color_token = advance().lexeme.substring(1); // strip leading #
