namespace GDiagram {
    public class ComponentDiagramParser : Object {
        ...
        private void parse_statement() throws Error {
            ...
            // database can be either a container (`database X { ... }`) or a
            // leaf element (`database "..." as db`). Route to the element
            // parser which handles both via its trailing `{` check, and also
            // recognises C4-style `database "label" <<stereo>> as alias [[`.
            ...
            // Check for file keyword (arrives as IDENTIFIER)
            if (check(TokenType.IDENTIFIER) && current_lexeme() == "file") {
                parse_identifier_element_declaration(ComponentType.FILE);
                return;
            }
        ...
        private void parse_component_declaration() {
        ...
        private Component? parse_element_inner() throws Error {
            ...
            // Optional <<stereotype>> before "as alias" (C4-PlantUML form)
            if (check(TokenType.STEREOTYPE)) {
                comp.stereotype = current_lexeme();
                advance();
                skip_whitespace();
            ...
            // Stereotype may also appear AFTER the alias
        ...
        private Component? parse_component_inner() {
            skip_whitespace();
        ...
        private void parse_interface_in_container(Component container) {
            ...
            // Add as child component for rendering inside the container
            var comp = new Component(name, ComponentType.INTERFACE);
            container.children.add(comp);

            // Also register in diagram.interfaces for relationship resolution
            var iface = new ComponentInterface(name);
        ...
        private void parse_element_declaration() throws Error {
            ...
            // Stereotype may also appear AFTER the alias (rare but allowed)
            if (comp.stereotype == null) {
                if (check(TokenType.STEREOTYPE)) {
                    comp.stereotype = current_lexeme();
                    advance();
        ...
        private bool is_direction_hint(string s) {
            string lower = s.down();
            return lower == "down" || lower == "up" || lower == "left" || lower == "right" ||
                   lower == "d" || lower == "u" || lower == "l" || lower == "r";
        }
        ...
        private void parse_relationship_from(string from_id) {
            ...
            advance(); // consume first arrow token

            // Handle direction-hinted arrows: -down->, -right->, -up->, etc.
            // Lexer splits "-down->" into MINUS "-", IDENTIFIER "down", ARROW_RIGHT "->"
            if (arrow == "-" && check(TokenType.IDENTIFIER) && is_direction_hint(current_lexeme())) {
                // Peek ahead to see if this is followed by an arrow token
                if (current + 1 < tokens.size) {
                    var next_type = tokens[current + 1].token_type;
                    if (next_type == TokenType.ARROW_RIGHT || next_type == TokenType.ARROW_RIGHT_DOTTED ||
                        next_type == TokenType.MINUS) {
                        advance(); // consume direction hint identifier
                        arrow = current_lexeme(); // the actual arrow part ("->", "-->", "-")
                        advance(); // consume actual arrow

                        // Handle the case "-down-" where MINUS is followed by ">" later
                        // But typically "-down->" produces MINUS + IDENTIFIER + ARROW_RIGHT
                    }
                }
            }
            // Handle ".down.>" → IDENTIFIER "." + IDENTIFIER "down" + DEPENDENCY "..>"
            // These patterns would be caught by the lexeme-based checks below

            // Detect arrow characteristics from the resolved arrow token
            if (arrow.contains("..") || arrow == "-->" || arrow == "<--") {
                is_dashed = true;
            ...
            if (check(TokenType.COLON)) {
                ...
                TokenType prev_type = TokenType.COLON;
                while (!is_at_end() && !check(TokenType.NEWLINE) && !check(TokenType.ENDUML)) {
                    var cur_type = tokens[current].token_type;
                    // Only insert a space between word-like tokens (identifiers,
                    // strings). Adjacent punctuation glues together so markup
                    // like "**bold**" or "<size:12>" isn't space-split into
                    // "* * b o l d * *".
                    if (label_sb.len > 0 && is_word_token(cur_type) && is_word_token(prev_type)) {
                        label_sb.append(" ");
                    }
        ...
        // True for tokens that benefit from a separator space when joined
        // back into a label. Used by parse_relationship_from to avoid
        // space-splitting punctuation like "**bold**" or "<size:12>".
        private bool is_word_token(TokenType t) {
        ...
        // (Lexer's scan_color() returns '#Color' as a single IDENTIFIER token)
        private void try_parse_color(Component comp) {
            if (check(TokenType.IDENTIFIER) && current_lexeme().has_prefix("#")) {
                string color_token = current_lexeme().substring(1); // strip leading #
                advance();
                if (is_hex_color(color_token)) {
                    comp.color = "#" + color_token;
                } else {
                    comp.color = color_token;
                }
