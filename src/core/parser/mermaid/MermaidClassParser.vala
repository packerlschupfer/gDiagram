namespace GDiagram {
    public class MermaidClassParser : Object {
        ...
        private void parse_statement() throws GLib.Error {
            ...
            // Standalone annotation: <<interface>> ClassName (the annotation word used to
            ...
            if (check(MermaidTokenType.ASYMMETRIC_START) &&
                ...
                peek_at(2).token_type == MermaidTokenType.IDENTIFIER) {
                parse_annotation();
                return;
            }
        ...
        private void parse_annotation() {
            ...
            string stereotype = advance().lexeme;
            if (check(MermaidTokenType.ASYMMETRIC_START)) advance();
            if (check(MermaidTokenType.ASYMMETRIC_START)) advance();
            if (!check(MermaidTokenType.IDENTIFIER)) return;
            var cls = diagram.get_or_create_class(advance().lexeme);
