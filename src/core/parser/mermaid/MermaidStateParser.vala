namespace GDiagram {
    public class MermaidStateParser : Object {
        ...
        private void parse_statement() throws GLib.Error {
            ...
            // classDef name fill:#f00,stroke:#333 — these lines used to become states
            if (check(MermaidTokenType.CLASS_DEF)) {
                advance();
                if (check(MermaidTokenType.IDENTIFIER)) {
                    string name = advance().lexeme;
                    diagram.class_defs.set(name, read_line_text());
                } else {
                    read_line_text();
                }
                return;
            ...
            if (check(MermaidTokenType.CLASS_KW)) {
                ...
                while (!check(MermaidTokenType.NEWLINE) && !is_at_end()) {
                    var t = advance();
                    if (t.token_type != MermaidTokenType.COMMA) ids.add(t.lexeme);
                }
                if (ids.size >= 2) {
                    string name = ids.remove_at(ids.size - 1);
                    foreach (string id in ids) {
                        var state = diagram.get_or_create_state(id);
                        assign_parent_if_new(state);
                        state.css_classes.add(name);
                    }
                }
                return;
            }
            ...
            if (check(MermaidTokenType.STYLE)) {
                advance();
                if (check(MermaidTokenType.IDENTIFIER)) {
                    var state = diagram.get_or_create_state(advance().lexeme);
                    assign_parent_if_new(state);
                    state.inline_style = read_line_text();
                } else {
                    read_line_text();
                }
                return;
        ...
        private void parse_state_declaration() throws GLib.Error {
            ...
            bool quoted = check(MermaidTokenType.STRING);
            string id = advance().lexeme;

            // Mermaid's `state "Description" as Id`: the quoted text is the label and the
            // name after 'as' the id (it used to become a state named after the label)
            if (quoted && check(MermaidTokenType.AS) &&
                peek_at(1).token_type == MermaidTokenType.IDENTIFIER) {
                advance(); // consume 'as'
                string description = id;
                id = advance().lexeme;
                diagram.get_or_create_state(id).description = description;
            }
            var state = diagram.get_or_create_state(id);
        ...
        private void parse_state_or_transition() throws GLib.Error {
            ...
            // "A:::hot" applies a classDef (it used to become the description ": : hot")
            read_class_shorthand(first_id);
            skip_whitespace_same_line();
        ...
        // ":::name" right after a state id
        private void read_class_shorthand(string state_id) {
            if (check(MermaidTokenType.COLON) &&
                peek_at(1).token_type == MermaidTokenType.COLON &&
                peek_at(2).token_type == MermaidTokenType.COLON &&
                peek_at(3).token_type == MermaidTokenType.IDENTIFIER) {
                advance(); advance(); advance();
                diagram.get_or_create_state(state_id).css_classes.add(advance().lexeme);
            }
        }

        // The rest of the line as written, without spaces between tokens (style specs)
