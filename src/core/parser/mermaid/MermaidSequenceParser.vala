namespace GDiagram {
    public class MermaidSequenceParser : Object {
        ...
        private void parse_statement() throws GLib.Error {
            ...
            // Else/And (alt/par section dividers) — skip the header line
            if (check(MermaidTokenType.ELSE) || check(MermaidTokenType.AND)) {
            ...
            // Skip accessibility annotations and 'box ... end' groupings
            if (check(MermaidTokenType.IDENTIFIER)) {
                ...
                // 'box [color] [title]' starts a participant group block;
                // skip just the header line — participants inside are parsed normally,
                // and the matching 'end' is consumed by the END handler above.
                if (kw == "box") {
                    while (!check(MermaidTokenType.NEWLINE) && !is_at_end()) {
                        advance();
                    }
                ...
                // 'create participant/actor X' — treat as participant declaration
                if (kw == "create") {
                    advance(); // consume 'create'
                    // Now expect participant/actor keyword or just identifier
                    if (check(MermaidTokenType.PARTICIPANT) || check(MermaidTokenType.ACTOR)) {
                        parse_participant();
                    } else if (check(MermaidTokenType.IDENTIFIER)) {
                        string id = advance().lexeme;
                        diagram.get_or_create_actor(id);
                        // skip rest of line
                        while (!check(MermaidTokenType.NEWLINE) && !is_at_end()) {
                            advance();
                        }
                ...
                // 'destroy X' — skip; the participant stays in the diagram
                if (kw == "destroy") {
                    while (!check(MermaidTokenType.NEWLINE) && !is_at_end()) {
                        advance();
                    }
            ...
            if (match(MermaidTokenType.OVER)) {
                ...
                if (match(MermaidTokenType.COMMA)) {
                    ...
                    to_actor = diagram.get_or_create_actor(advance().lexeme);
                       ...
                       (check(MermaidTokenType.IDENTIFIER) && peek().lexeme.down() == "left")) {
                advance(); // consume "left" or LEFT_OF
                if (check(MermaidTokenType.IDENTIFIER) && peek().lexeme.down() == "of") {
                    advance(); // consume "of"
                }
                is_right = false;
                if (!check(MermaidTokenType.IDENTIFIER)) {
                ...
                from_actor = diagram.get_or_create_actor(advance().lexeme);
                       ...
                       (check(MermaidTokenType.IDENTIFIER) && peek().lexeme.down() == "right")) {
                advance(); // consume "right" or RIGHT_OF
                if (check(MermaidTokenType.IDENTIFIER) && peek().lexeme.down() == "of") {
                    advance(); // consume "of"
                }
                is_right = true;
        ...
        private void parse_loop() throws GLib.Error {
            ...
            // Parse optional condition/label (with or without leading colon)
            if (match(MermaidTokenType.COLON)) {
                var cond_parts = new StringBuilder();
                while (!check(MermaidTokenType.NEWLINE) && !is_at_end()) {
                    if (cond_parts.len > 0) cond_parts.append(" ");
                ...
                if (cond.length > 0) loop.condition = cond;
            }

            skip_newlines();

            // Record which message indices fall inside this loop
        ...
        private void parse_message() throws GLib.Error {
            ...
            // Check for activation/deactivation marker (+/-) before destination actor
            bool activate = false;
            ...
            message.is_activation = activate;
            message.is_deactivation = deactivate;
