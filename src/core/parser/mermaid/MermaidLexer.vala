namespace GDiagram {
    public class MermaidLexer : Object {
        ...
        private static void init_keywords() {
            ...
            keywords.insert("stateDiagram",    MermaidTokenType.STATE_DIAGRAM);
            ...
            keywords.insert("linkStyle", MermaidTokenType.LINK_STYLE);
        ...
        private MermaidToken scan_comment(int start_column) {
            ...
            // Check for %%{ ... }%% init/config block (possibly multi-line)
            if (!is_at_end() && peek() == '{') {
                while (!is_at_end()) {
                    unichar ch = advance();
                    if (ch == '}' && peek() == '%') {
                ...
                string block_text = source.substring(start, current - start);
                return new MermaidToken(MermaidTokenType.COMMENT, block_text, line, start_column);
            }
        ...
        private MermaidToken scan_arrow_or_line(int start_column) {
            ...
            if (peek() == ')') {
                // Sequence-diagram async arrows: -) and --)
                sb.append_c((char)advance());
                if (dash_count == 1) {
                    return new MermaidToken(MermaidTokenType.SEQ_SOLID_OPEN, sb.str, line, start_column);
                } else {
        ...
        private MermaidToken scan_number(int start_column) {
            var sb = new StringBuilder();
            sb.append(source.substring(start, current - start));
        ...
        private MermaidToken scan_identifier(int start_column) {
            ...
            while (peek() == '-') {
                // Disambiguate sequence-diagram arrows (-x, -), -o) from hyphenated
                // identifiers. If the dash starts one of those arrows AND the arrow is
                // immediately followed by another identifier character (i.e. there is
                // no space between arrow and target actor like "Auth-xGateway"), treat
                // the dash as the start of an arrow and stop the identifier here.
                int after_dash = current + 1;
                unichar next = char_at(ref after_dash);
                unichar after = char_at(ref after_dash);
                if ((next == 'x' || next == ')' || next == 'o') &&
                    (after.isalnum() || after == '_')) {
                    break;
                }
        ...
        // The source is UTF-8 and Vala string indexing is by byte: reading single bytes
        // turned "Ä[Grüße]" into the Latin-1 letters of its encoding and split the label
        // into nodes "Gr" and "e". Characters are decoded whole; an invalid byte reads as
        // itself.
        private unichar char_at(ref int pos) {
            if (pos >= source.length) return '\0';
            unichar c;
            int next = pos;
            if (!source.get_next_char(ref next, out c)) {
                c = (unichar) source[pos];
                next = pos + 1;
            }
            pos = next;
            return c;
