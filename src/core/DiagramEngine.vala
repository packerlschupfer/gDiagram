namespace GDiagram {
    ...
    public class DiagramEngine : Object {
        ...
        /** Errors from the most recent preprocess(), e.g. an unresolved !include. */
        public Gee.ArrayList<PreprocessorError> preprocessor_errors {
        ...
        // `base_path` (the document's path or directory) resolves relative
        // !include paths; without it they resolve against the working directory.
        public ParseResult parse(string source, string? doc_filename, string? base_path = null) {
        ...
        private void parse_plantuml_ast(ParseResult r, string source) {
            switch (r.diagram_type) {
                ...
                case DiagramType.ANCESTRY:
                    ...
                    break;
                case DiagramType.MERMAID_PACKET: {
                    // @startpacketdiag inside a PlantUML document
                    var d = mermaid_packet_parser.parse_packetdiag(source);
                    r.ast = d; r.errors = d.errors; break;
                }
                default:
                    r.unsupported = true;
        ...
        // Ditaa, Salt, Chen, EBNF, Regex, Ancestry, packetdiag) that all follow
        // the same pattern: parse source → generate DOT → render via Graphviz.
        // The DOT comes from generate_plantuml_dot(), the single dispatch shared
        // with `-f dot`, the LSP SVG command and file export. Produces no AST
        // for the outline (ast == null).
        private RenderResult render_generic_plantuml(string source, DiagramType type) {
            if (!is_generic_plantuml_type(type)) {
                return unknown("Unsupported diagram type");
            }
            string? dot_output = generate_plantuml_dot(source, type);
            if (dot_output == null || dot_output.length == 0) {
                return unknown("Failed to generate diagram");
            }

            uint8[]? svg_data = render_dot_to_svg(dot_output);
            if (svg_data == null) {
        ...
        // The PlantUML types rendered through generate_plantuml_dot() alone.
        // @startpacketdiag is detected as MERMAID_PACKET but, in a PlantUML
        // document, is drawn in PlantUML's packetdiag style.
        private static bool is_generic_plantuml_type(DiagramType type) {
            switch (type) {
                case DiagramType.DOT_DIAGRAM:
                case DiagramType.TREE:
                case DiagramType.BOARD:
        ...
        // File export for the generic types: the same DOT → SVG render as the
        // preview, then written as SVG, PDF or PNG.
        private bool export_generic_plantuml(string processed, DiagramType type, string format, string filename) {
            uint8[]? svg = render_dot_to_svg(generate_plantuml_dot(processed, type));
            if (svg == null) return false;
            if (format == "svg") return RenderUtils.write_svg_to_file(svg, filename);
            if (format == "pdf") return RenderUtils.export_svg_to_pdf(svg, filename);
            var surface = RenderUtils.svg_to_surface(svg);
        ...
        public bool export_to_png(string source, string? doc_filename, string? base_path, string filename) {
            ...
            switch (diagram_type) {
                ...
                case DiagramType.ANCESTRY:
                case DiagramType.MERMAID_PACKET:
                    return export_generic_plantuml(processed_source, diagram_type, "png", filename);
                default:
        ...
        public bool export_to_svg(string source, string? doc_filename, string? base_path, string filename) {
            ...
            switch (diagram_type) {
                ...
                case DiagramType.ANCESTRY:
                case DiagramType.MERMAID_PACKET:
                    return export_generic_plantuml(processed_source, diagram_type, "svg", filename);
                default:
                    var seq_diagram = parser.parse(processed_source);
                    if (seq_diagram.has_errors() || seq_diagram.participants.size == 0) return false;
        ...
        public bool export_to_pdf(string source, string? doc_filename, string? base_path, string filename) {
            ...
            switch (diagram_type) {
                ...
                case DiagramType.ANCESTRY:
                case DiagramType.MERMAID_PACKET:
                    return export_generic_plantuml(processed_source, diagram_type, "pdf", filename);
                default:
                    var seq_diagram = parser.parse(processed_source);
                    if (seq_diagram.has_errors() || seq_diagram.participants.size == 0) return false;
        ...
        private string? generate_plantuml_dot(string processed, DiagramType type) {
            ...
            switch (type) {
                ...
                case DiagramType.MERMAID_PACKET: {
                    var d = mermaid_packet_parser.parse_packetdiag(processed);
        ...
        private uint8[]? render_dot_to_svg(string? dot) {
            ...
            uint8[] svg_data;
            int ret = RenderUtils.render_data(ctx, graph, "svg", out svg_data);
