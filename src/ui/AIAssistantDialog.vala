namespace GDiagram {
    public class AIAssistantDialog : Adw.Dialog {
        ...
        construct {
            ...
            string scheme_id = adw_style.dark ? "Adwaita-dark" : "Adwaita";
            var scheme = style_manager.get_scheme(scheme_id);
            if (scheme != null) {
                result_buffer.style_scheme = scheme;
            }
            adw_style.notify["dark"].connect(() => {
                string sid = adw_style.dark ? "Adwaita-dark" : "Adwaita";
                var s = style_manager.get_scheme(sid);
                if (s != null) result_buffer.style_scheme = s;
            });
