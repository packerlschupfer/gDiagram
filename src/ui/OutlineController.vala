namespace GDiagram {
    ...
    public class OutlineController : Object {
        ...
        private void add_class_to_outline(UmlClass c, OutlineNode? parent) {
            ...
            add_outline_item(c.name, icon, parent);
        }
        ...
        private void update_outline_from_gantt_diagram(PumlGanttDiagram diagram) {
            ...
            foreach (var task in diagram.tasks) {
                if (task.section != null && task.section != current_section) {
                    ...
                    section_node = add_outline_item(task.section, "folder-symbolic");
                }
                string icon = task.is_milestone ? "starred-symbolic" : "view-list-bullet-symbolic";
                OutlineNode? parent = task.section != null ? section_node : null;
                add_outline_item("%s (%d days)".printf(task.name, task.duration_days), icon, parent);
            }
        }
        ...
        private void update_outline_from_mermaid_git_graph(MermaidGitGraph diagram) {
            ...
            foreach (var branch in diagram.branches) {
                var branch_node = add_outline_item(branch.name, "vcs-branch-symbolic");
                foreach (var commit in branch.commits) {
                    string lbl = (commit.tag != null && commit.tag.length > 0)
                        ? commit.id + " [" + commit.tag + "]"
                        : commit.id;
                    add_outline_item(lbl, "vcs-commit-symbolic", branch_node);
                }
            }
        }
    ...
    internal class OutlineRow : Gtk.ListBoxRow {
        ...
        public OutlineRow(OutlineNode node, int depth, bool collapsed) {
            ...
            if (node.children.size > 0) {
                ...
                expander = new Gtk.Image.from_icon_name(collapsed ? "pan-end-symbolic" : "pan-down-symbolic");
            ...
            var icon = new Gtk.Image.from_icon_name(node.icon_name);
        ...
        private void on_expander_pressed(Gtk.GestureClick click, int n_press, double x, double y) {
            ...
            expander.icon_name = is_collapsed ? "pan-end-symbolic" : "pan-down-symbolic";
