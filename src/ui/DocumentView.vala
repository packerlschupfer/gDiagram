namespace GDiagram {
    ...
    public class DocumentView : Gtk.Box {
        ...
        // Sidebars: outline and element palette on the left, properties of the clicked element
        // on the right. Adw overlay split views fold over the content on narrow windows.
        private Adw.OverlaySplitView left_split;
        private Adw.OverlaySplitView right_split;
        private Gtk.Box sidebar_box;
        private Adw.ViewStack sidebar_stack;
        private Gtk.SearchEntry outline_filter;
        // Filled by the element palette and the properties panel
        public Gtk.Box palette_page { get; private set; }
        public Gtk.Box properties_page { get; private set; }
        private ElementPalette element_palette;
        private PropertiesPanel properties_panel;
        ...
        construct {
            ...
            // Left sidebar: outline and element palette
            setup_outline_view();
            ...
            diagram_search.search_changed.connect(on_diagram_search_changed);
            diagram_search.activate.connect(on_diagram_search_activate);
            ...
            split_toggle.clicked.connect(toggle_split_orientation);
            ...
            // The bar's width (zoom buttons, find entry, toggles) became the preview column's
            // minimum width. Gtk.Paned lets an end child below its minimum run off the right
            // edge, so the diagram was centred partly off-screen. Clip the bar instead.
            var zoom_clip = new Gtk.ScrolledWindow();
            zoom_clip.hscrollbar_policy = Gtk.PolicyType.EXTERNAL;
            zoom_clip.vscrollbar_policy = Gtk.PolicyType.NEVER;
            zoom_clip.propagate_natural_width = true;
            zoom_clip.propagate_natural_height = true;
            zoom_clip.child = zoom_bar;
            preview_box.append(zoom_clip);

            main_paned.start_child = editor_box;
            main_paned.end_child = preview_box;
            main_paned.position = 550;

            // Right sidebar: properties of the element clicked in the preview, hidden until
            // toggled from the header bar
            properties_page = new Gtk.Box(Gtk.Orientation.VERTICAL, 0);
            properties_panel = new PropertiesPanel(source_buffer);
            properties_panel.source_edited.connect(replace_source_text);
            properties_panel.go_to_source.connect((id, line) => {
                if (line > 0) {
                    navigate_to_line(line);
                } else {
                    search_and_navigate(id);
                }
            });
            properties_page.append(properties_panel);

            right_split = new Adw.OverlaySplitView();
            right_split.sidebar_position = Gtk.PackType.END;
            right_split.content = main_paned;
            right_split.sidebar = properties_page;
            right_split.show_sidebar = false;
            // Pinned: folding would hide the sidebar and unfolding show it again whatever
            // the user chose; set_narrow() restores the chosen state instead
            right_split.pin_sidebar = true;
            right_split.min_sidebar_width = 240;
            right_split.max_sidebar_width = 360;
            right_split.notify["show-sidebar"].connect(() => sidebars_changed());

            left_split = new Adw.OverlaySplitView();
            left_split.sidebar_position = Gtk.PackType.START;
            left_split.sidebar = sidebar_box;
            left_split.content = right_split;
            left_split.show_sidebar = true;
            left_split.pin_sidebar = true;
            left_split.min_sidebar_width = 180;
            left_split.max_sidebar_width = 320;
            left_split.hexpand = true;
            left_split.vexpand = true;
            left_split.notify["show-sidebar"].connect(() => sidebars_changed());

            this.append(left_split);

            // Header bar undo/redo buttons follow the editor's undo stack
            source_buffer.notify["can-undo"].connect(() => undo_state_changed());
            source_buffer.notify["can-redo"].connect(() => undo_state_changed());
        ...
        private void setup_outline_view() {
            ...
            // Header: Outline / Palette switcher and hide button
            ...
            sidebar_stack = new Adw.ViewStack();
            sidebar_stack.vexpand = true;

            var switcher = new Adw.ViewSwitcher();
            switcher.stack = sidebar_stack;
            ...
            hide_btn.tooltip_text = "Hide Sidebar (Ctrl+\\)";
            ...
            // Outline filter: matching entries and the parents leading to them
            outline_filter = new Gtk.SearchEntry();
            outline_filter.placeholder_text = "Filter outline";
            outline_filter.margin_start = 6;
            outline_filter.margin_end = 6;
            outline_filter.margin_bottom = 6;
            outline_filter.search_changed.connect(() => outline_controller.set_filter(outline_filter.text));
            sidebar_box.append(outline_filter);
            // Only the outline page is filtered
            sidebar_stack.notify["visible-child-name"].connect(() => {
                outline_filter.visible = sidebar_stack.visible_child_name == "outline";
            });
            ...
            sidebar_stack.add_titled_with_icon(scroll, "outline", "Outline", "view-list-symbolic");

            // Element palette page; the palette fills palette_page
            palette_page = new Gtk.Box(Gtk.Orientation.VERTICAL, 0);
            element_palette = new ElementPalette();
            element_palette.vexpand = true;
            element_palette.snippet_activated.connect((entry) => {
                insert_snippet(entry.snippet, entry.select);
            });
            palette_page.append(element_palette);
            sidebar_stack.add_titled_with_icon(palette_page, "palette", "Palette", "applications-graphics-symbolic");
        ...
        private void on_element_clicked(string element_name, int source_line) {
            properties_panel.show_element(element_name, source_line);
        ...
        private void rebuild_breadcrumbs() {
            ...
            for (int i = 0; i < nav_history.size; i++) {
                ...
                btn.set_data<int>("gdiagram-history-index", i);
                btn.clicked.connect(on_breadcrumb_clicked);
        ...
        private void on_element_context_menu(string element_name, int source_line, double x, double y) {
            ...
            // Every right-click parented a new popover to the preview for good, and its
            // actions' closures hold this view: unparent it once it is closed
            popover.closed.connect(on_context_popover_closed);
        ...
        // ==================== Handlers without closures ====================
        // Lambdas that capture a local variable hold a strong reference to this view from
        // the child widget they're connected to. Those reference cycles kept every closed
        // tab alive, so these handlers take what they need from the emitting widget.

        private void on_diagram_search_changed(Gtk.SearchEntry entry) {
        ...
        private void on_diagram_search_activate(Gtk.SearchEntry entry) {
            string query = entry.text.strip().down();
            if (query.length >= 2) {
        ...
        private void on_breadcrumb_clicked(Gtk.Button button) {
            navigate_breadcrumb_to(button.get_data<int>("gdiagram-history-index"));
        }

        private void on_case_toggled(Gtk.ToggleButton button) {
            search_settings.case_sensitive = button.active;
            update_search_status();
        }

        private void on_regex_toggled(Gtk.ToggleButton button) {
            search_settings.regex_enabled = button.active;
            update_search_status();
        }

        private static void on_context_popover_closed(Gtk.Popover popover) {
            Idle.add(() => {
                popover.unparent();
                return false;
            });
        ...
        private void setup_search_bar(Gtk.Box parent) {
            ...
            // Find and replace on two rows: on one row the bar needed the width of both
            // entries plus every button and was clipped in a narrow editor column
            var search_box = new Gtk.Box(Gtk.Orientation.VERTICAL, 4);
            ...
            find_row.add_css_class("toolbar");
            var replace_row = new Gtk.Box(Gtk.Orientation.HORIZONTAL, 6);
            replace_row.add_css_class("toolbar");
            ...
            search_entry.width_chars = 8;

            // Replace entry
            replace_entry = new Gtk.Entry();
            replace_entry.placeholder_text = "Replace...";
            replace_entry.width_chars = 8;
            replace_entry.hexpand = true;

            // Status label ("3 of 12"): room for the count, shortened only past that; with
            // ellipsizing alone the expanding entry took all the width and left "..."
            search_status_label = new Gtk.Label("");
            search_status_label.add_css_class("dim-label");
            search_status_label.ellipsize = Pango.EllipsizeMode.END;
            search_status_label.width_chars = 8;
            ...
            // A text label like the ".*" toggle: format-text-uppercase-symbolic is not in
            // the Adwaita icon theme and showed as an empty square
            var case_btn = new Gtk.ToggleButton();
            case_btn.label = "Aa";
            case_btn.tooltip_text = "Case Sensitive";
            case_btn.toggled.connect(on_case_toggled);

            // Regex toggle
            var regex_btn = new Gtk.ToggleButton();
            regex_btn.label = ".*";
            regex_btn.tooltip_text = "Regular Expression";
            regex_btn.toggled.connect(on_regex_toggled);

            // Close first, so a column too narrow for the bar clips a toggle, not the way out;
            // the option toggles sit on the shorter replace row
            find_row.append(close_btn);
            find_row.append(search_entry);
            find_row.append(search_status_label);
            ...
            replace_row.append(case_btn);
            replace_row.append(regex_btn);

            // The bar's width (two entries, buttons) became the editor column's minimum width,
            // even while hidden. Gtk.Paned squeezes a start child below its minimum by moving
            // it left under the sidebar: the gutter and line starts vanished. Clip it instead.
            var search_clip = new Gtk.ScrolledWindow();
            search_clip.hscrollbar_policy = Gtk.PolicyType.EXTERNAL;
            search_clip.vscrollbar_policy = Gtk.PolicyType.NEVER;
            search_clip.propagate_natural_width = true;
            search_clip.propagate_natural_height = true;
            search_clip.child = search_box;
            search_revealer.child = search_clip;
        ...
        public void show_search() {
            ...
            if (search_context == null) {
                ...
                // Matches are counted asynchronously: the count is -1 ("...") right after a
                // change, and nothing refreshed the label once counting finished
                search_context.notify["occurrences-count"].connect(on_occurrences_counted);
                search_context.notify["regex-error"].connect(update_search_status);
        ...
        private void replace_all() {
            ...
            try {
                if (search_context.regex_error != null) {
                    update_search_status();
                    return;
                }
                int count = (int) search_context.replace_all(replace_entry.text, -1);
                // The recount after the replacement ("No results") must not overwrite this
                replace_status = "Replaced %d".printf(count);
                search_status_label.label = replace_status;
                search_status_label.tooltip_text = null;
            } catch (Error e) {
                warning("Replace all error: %s", e.message);
            }
        }

        // "Replaced N" stays up until the search itself changes or the user searches again
        private string? replace_status = null;

        private void on_occurrences_counted() {
            if (replace_status != null) return;
            update_search_status();
        }
        ...
        private void update_search_status() {
            ...
            if (search_context.regex_error != null) {
                search_status_label.label = "Invalid regex";
                search_status_label.tooltip_text = search_context.regex_error.message;
                return;
            }
        ...
        private void render_preview() {
            ...
            element_palette.set_diagram_type(current_diagram_type, current_diagram_format);
        ...
        private void apply_render_result(RenderResult result, string source) {
            switch (result.status) {
                ...
                default:
                    ...
                    properties_panel.refresh(result.diagram_type, result.ast, engine.last_regions);
                    if (current_diagram_format == DiagramFormat.MERMAID) {
                        set_and_cache_surface(result.surface, source);
                        // Mermaid previews never got click regions: clicking a node did not
                        // navigate to the source or reach the properties panel
        ...
        // What the user chose for each sidebar on a wide window. On a narrow window the
        // sidebars are overlays opened on demand, which doesn't change the choice.
        private bool outline_wanted = true;
        private bool properties_wanted = false;

        public void toggle_outline_visibility() {
            left_split.show_sidebar = !left_split.show_sidebar;
            if (!left_split.collapsed) outline_wanted = left_split.show_sidebar;
        }

        public void toggle_properties_visibility() {
            right_split.show_sidebar = !right_split.show_sidebar;
            if (!right_split.collapsed) properties_wanted = right_split.show_sidebar;
        }

        // Header bar toggles follow these
        public signal void sidebars_changed();
        public bool sidebar_visible { get { return left_split.show_sidebar; } }
        public bool properties_visible { get { return right_split.show_sidebar; } }

        // Narrow windows: sidebars overlay the content instead of taking width
        public void set_narrow(bool narrow) {
            if (left_split.collapsed == narrow && right_split.collapsed == narrow) return;
            left_split.collapsed = narrow;
            right_split.collapsed = narrow;
            // Folded sidebars start hidden; unfolding brings back the user's choice
            left_split.show_sidebar = !narrow && outline_wanted;
            right_split.show_sidebar = !narrow && properties_wanted;
        }

        // ==================== Undo / Redo ====================

        public signal void undo_state_changed();
        public bool can_undo { get { return source_buffer.can_undo; } }
        public bool can_redo { get { return source_buffer.can_redo; } }

        public void undo() {
            if (source_buffer.can_undo) source_buffer.undo();
        }

        public void redo() {
            if (source_buffer.can_redo) source_buffer.redo();
        }

        // Element palette insertion (see SourceEdit.insert_snippet), one undo step
        public void insert_snippet(string text, string? select) {
            SourceEdit.insert_snippet(source_buffer, text, select, element_palette.insert_type);
            source_view.scroll_to_mark(source_buffer.get_insert(), 0.1, false, 0, 0);
            source_view.grab_focus();
        }

        // Properties panel edits: ONE undoable step that only touches the changed text,
        // so the cursor and scroll position stay where they were
        public void replace_source_text(string new_text) {
            SourceEdit.replace_text(source_buffer, new_text);
        }
        ...
        /**
         * The document's file path, the preprocessor's base for relative includes
         * (it resolves against the file's directory). A file path rather than the
         * directory lets it recognise an include cycle back to this document.
