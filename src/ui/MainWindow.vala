namespace GDiagram {
    ...
    public class MainWindow : Adw.ApplicationWindow {
        ...
        [GtkChild]
        private unowned Gtk.ToggleButton properties_toggle;
        [GtkChild]
        private unowned Adw.Breakpoint narrow_breakpoint;

        // Header bar state follows the selected document; handler ids of the signals
        // connected on it, disconnected when another tab is selected
        private DocumentView? header_view = null;
        private ulong[] header_handlers = {};
        ...
        construct {
            ...
            ActionEntry[] action_entries = {
                ...
                { "toggle-properties", this.on_toggle_properties },
                { "undo", this.on_undo },
                { "redo", this.on_redo },
            };
            ...
            // Keyboard shortcuts for these actions are registered in Application's construct

            // Header bar: title/path, undo/redo and sidebar toggles follow the selected tab
            tab_view.notify["selected-page"].connect(on_selected_page_changed);
            sidebar_toggle.toggled.connect(() => {
                if (header_view != null && header_view.sidebar_visible != sidebar_toggle.active) {
                    header_view.toggle_outline_visibility();
                }
            });
            properties_toggle.toggled.connect(() => {
                if (header_view != null && header_view.properties_visible != properties_toggle.active) {
                    header_view.toggle_properties_visibility();
                }
            });

            // Narrow windows fold the sidebars of every open document, and of ones opened later
            narrow_breakpoint.apply.connect(() => set_narrow(true));
            narrow_breakpoint.unapply.connect(() => set_narrow(false));
            tab_view.page_attached.connect((page, position) => {
                var view = page.child as DocumentView;
                if (view != null) view.set_narrow(narrow);
            });
        ...
        private void on_selected_page_changed() {
            if (header_view != null) {
                foreach (ulong id in header_handlers) {
                    if (SignalHandler.is_connected(header_view, id)) {
                        SignalHandler.disconnect(header_view, id);
                    } else if (SignalHandler.is_connected(header_view.document, id)) {
                        SignalHandler.disconnect(header_view.document, id);
                    }
                }
            }
            header_handlers = {};
            var page = tab_view.selected_page;
            header_view = page != null ? page.child as DocumentView : null;
            if (header_view != null) {
                var doc = header_view.document;
                header_handlers += header_view.undo_state_changed.connect(update_header);
                header_handlers += header_view.sidebars_changed.connect(update_header);
                header_handlers += doc.notify["title"].connect(update_header);
                header_handlers += doc.notify["file"].connect(update_header);
                header_handlers += doc.notify["modified"].connect(update_header);
            }
            update_header();
        }
        ...
        private void update_header() {
            var page = tab_view.selected_page;
            var view = header_view;
            bool has_doc = view != null;

            if (has_doc) {
                var doc = view.document;
                // "• Title" while unsaved, like the tab indicator
                window_title.title = doc.modified ? "• " + doc.title : doc.title;
                window_title.subtitle = doc.file != null ? display_path(doc.file) : "Not saved";
            } else {
                // Git repository tab or no tab at all
                window_title.title = page != null ? page.title : "gDiagram";
                window_title.subtitle = "";
            }

            ((SimpleAction) lookup_action("undo")).set_enabled(has_doc && view.can_undo);
            ((SimpleAction) lookup_action("redo")).set_enabled(has_doc && view.can_redo);
            sidebar_toggle.sensitive = has_doc;
            properties_toggle.sensitive = has_doc;
            if (has_doc) {
                // The toggled handlers compare with the view first, so this does not loop
                sidebar_toggle.active = view.sidebar_visible;
                properties_toggle.active = view.properties_visible;
            }
        }

        // "~/docs/diagram.puml" rather than the full home path
        private string display_path(File file) {
            string? path = file.get_path();
            if (path == null) return file.get_uri();
        ...
        private void set_narrow(bool value) {
            narrow = value;
            for (int i = 0; i < tab_view.n_pages; i++) {
                var view = tab_view.get_nth_page(i).child as DocumentView;
                if (view != null) view.set_narrow(value);
            }
        }

        private void on_toggle_properties() {
            if (header_view != null) header_view.toggle_properties_visibility();
        }

        private void on_undo() {
            if (header_view != null) header_view.undo();
        }

        private void on_redo() {
            if (header_view != null) header_view.redo();
        }

        // Tab title and indicator follow the document. A method handler looking the page up:
        // closures capturing the page made the document hold its page, which holds the view
        // that holds the document, so a closed tab was never freed.
        private void watch_document(Document doc) {
            doc.notify["title"].connect(on_document_changed);
            doc.notify["modified"].connect(on_document_changed);
            doc.notify["git-dirty"].connect(on_document_changed);
        }

        private void on_document_changed(Object obj, ParamSpec pspec) {
            var doc = (Document) obj;
            for (int i = 0; i < tab_view.n_pages; i++) {
                var page = tab_view.get_nth_page(i);
                var view = page.child as DocumentView;
                if (view == null || view.document != doc) continue;
                if (pspec.name == "title") {
                    page.title = doc.title;
                } else {
                    update_tab_indicator(page, doc);
                }
                return;
            }
        }
        ...
        public void open_file_with_history(File file, Gee.ArrayList<File>? history) {
            ...
            doc.load_from_file.begin(file, (obj, res) => {
                try {
                    ...
                    // Close an untouched "Untitled" scratch tab if one exists — after moving
                    // focus to the new document and letting GTK process it. Closing the tab
                    // while its editor still had keyboard focus crashed in GTK's Wayland
                    // text-input handling (gtk_widget_get_display on the destroyed widget)
                    // whenever the window was really focused, e.g. when opened from Nautilus
