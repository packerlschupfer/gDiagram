        ...
        private int handle_export(string input_path, string output_path, string format, bool scale_output = false) {
            try {
                ...
                foreach (var err in engine.preprocessor_errors) {
                    bool is_include = err.message.has_prefix("Cannot resolve include") ||
                                      err.message.has_prefix("Standard library include not available");
                ...
                int status = do_export(engine, content, input_path, input_path,
                                       output_path, format, scale_output);
