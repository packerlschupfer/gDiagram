namespace GDiagram.Tests {
    public class RenderUtilsTests {
        ...
        // Named colours outside a short list got white text, unreadable on light fills
        public static void test_contrast_named_colors() {
            assert(RenderUtils.contrast_text("DarkSeaGreen") == "#000000");
            assert(RenderUtils.contrast_text("YellowGreen") == "#000000");
            assert(RenderUtils.contrast_text("Tan") == "#000000");
            assert(RenderUtils.contrast_text("#LightGoldenRodYellow") == "#000000");
            assert(RenderUtils.contrast_text("Navy") == "#FFFFFF");
            assert(RenderUtils.contrast_text("DarkSlateGray") == "#FFFFFF");
            assert(RenderUtils.contrast_text("notacolor") == "#FFFFFF");
        }

        // A 6-letter name was parsed as hex digits, and "#FFF" short hex fell through to white
        public static void test_contrast_six_letter_names_and_short_hex() {
            assert(RenderUtils.contrast_text("Yellow") == "#000000");
            assert(RenderUtils.contrast_text("Silver") == "#000000");
            assert(RenderUtils.contrast_text("Maroon") == "#FFFFFF");
            assert(RenderUtils.contrast_text("Purple") == "#FFFFFF");
            assert(RenderUtils.contrast_text("#FFF") == "#000000");
            assert(RenderUtils.contrast_text("#000") == "#FFFFFF");
            assert(RenderUtils.contrast_text("#FFFFFF") == "#000000");
        }

        // ── gradients ────────────────────────────────────────────

        static void assert_gradient(string value, string first, string second, int angle) {
            string f, s;
            int a;
            if (!RenderUtils.parse_gradient(value, out f, out s, out a) || f != first || s != second || a != angle) {
                stderr.printf("parse_gradient(%s) = %s / %s / %d, want %s / %s / %d\n",
                              value, f, s, a, first, second, angle);
                assert_not_reached();
            }
        }

        static void assert_no_gradient(string value) {
            string f, s;
            int a;
            if (RenderUtils.parse_gradient(value, out f, out s, out a)) {
                stderr.printf("parse_gradient(%s) took a plain colour as %s / %s\n", value, f, s);
                assert_not_reached();
            }
        }

        // PlantUML draws "#c1<sep>c2" as a two-colour gradient; only the first colour was drawn.
        // Angles match PlantUML 1.2026.1's SVG linearGradient directions.
        public static void test_parse_gradient_separators() {
            assert_gradient("#red|green", "red", "green", 0);
            assert_gradient("#red-green", "red", "green", 270);
            assert_gradient("#red/green", "red", "green", 315);
            assert_gradient("#red\\green", "red", "green", 45);
            assert(RenderUtils.gradient_angle_for_separator("|") == 0);
            assert(RenderUtils.gradient_angle_for_separator("-") == 270);
            assert(RenderUtils.gradient_angle_for_separator("/") == 315);
            assert(RenderUtils.gradient_angle_for_separator("\\") == 45);
        }

        public static void test_parse_gradient_hex_and_names() {
            // Second colour hex without "#"
            assert_gradient("#blue\\9932CC", "blue", "#9932CC", 45);
            assert_gradient("#FF0000-00FF00", "#FF0000", "#00FF00", 270);
            assert_gradient("#FF0000|green", "#FF0000", "green", 0);
            assert_gradient("#red|#00FF00", "red", "#00FF00", 0);
            assert_gradient("#F00-0F0", "#FF0000", "#00FF00", 270);
            // skinparam values carry no "#"
            assert_gradient("AntiqueWhite/Gold", "AntiqueWhite", "Gold", 315);
            assert_gradient("Wheat|CornflowerBlue", "Wheat", "CornflowerBlue", 0);
            // Component skinparam values arrive with the tokens space-joined
            assert_gradient("white | gray", "white", "gray", 0);
            assert(RenderUtils.sanitize_color("white | gray") == "white");
            assert(RenderUtils.sanitize_color("#FF0000-00FF00") == "#FF0000");
            // Inline style suffix: only the part before ";" is the fill
            assert_gradient("#pink-red;line:blue;text:red", "pink", "red", 270);
            assert(RenderUtils.fill_color("#GreenYellow/LightGoldenRodYellow") == "GreenYellow:LightGoldenRodYellow");
            assert(RenderUtils.gradient_attr("#red-green") == ", gradientangle=270");
            assert(RenderUtils.gradient_stmt("#red/green") == "gradientangle=315;");
            assert(RenderUtils.gradient_html_attr("#red|green") == " GRADIENTANGLE=\"0\"");
        }

        // Plain colours are left to sanitize_color, which keeps giving borders and fonts one colour
        public static void test_parse_gradient_plain_colors() {
            assert_no_gradient("#9932CC");
            assert_no_gradient("#LightGoldenRodYellow");
            assert_no_gradient("red");
            assert_no_gradient("#pink;line:red;text:red");
            assert_no_gradient("#red-");
            assert_no_gradient("-red");
            assert_no_gradient("#line.dashed");
            assert(RenderUtils.gradient_attr("#pink") == "");
            assert(RenderUtils.gradient_stmt(null) == "");
            assert(RenderUtils.fill_color("#pink") == "pink");
            assert(RenderUtils.fill_color("#9932CC") == "#9932CC");
            assert(RenderUtils.sanitize_color("#red-green") == "red");
            assert(RenderUtils.sanitize_color("AntiqueWhite/Gold") == "AntiqueWhite");
        }

        // Text on a gradient is contrasted to the mean of both colours
        public static void test_contrast_gradient_fill() {
            assert(RenderUtils.contrast_text("Gray:White") == "#000000");
            assert(RenderUtils.contrast_text("Gray") == "#FFFFFF");
            assert(RenderUtils.contrast_text("red:green") == "#FFFFFF");
            assert(RenderUtils.contrast_text("White:#FFFF00") == "#000000");
            assert(RenderUtils.contrast_text("Black:notacolor") == "#FFFFFF");
        }
        ...
        public static void test_fuzz_escape_id_unicode() {
            // Non-ASCII letters and digits are valid in DOT ids and are kept, so "Ä" and "Ü"
            // stay two nodes; other characters (symbols, spaces) still become underscores.
            assert(RenderUtils.escape_id("日本語") == "日本語");
            assert(RenderUtils.escape_id("Ä") != RenderUtils.escape_id("Ü"));
            string r = RenderUtils.escape_id("a→b c");
            assert(r == "a_b_c");
        }
        ...
        public static void main(string[] args) {
            ...
            Test.add_func("/render_utils/contrast-named-colors", test_contrast_named_colors);
            Test.add_func("/render_utils/contrast-six-letter-short-hex", test_contrast_six_letter_names_and_short_hex);
            Test.add_func("/render_utils/gradient-separators", test_parse_gradient_separators);
            Test.add_func("/render_utils/gradient-hex-and-names", test_parse_gradient_hex_and_names);
            Test.add_func("/render_utils/gradient-plain-colors", test_parse_gradient_plain_colors);
            Test.add_func("/render_utils/contrast-gradient-fill", test_contrast_gradient_fill);
            Test.run();
        }
    }
}
