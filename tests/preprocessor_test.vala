...
@enduml""";
        ...
        public static void test_include_cycle_back_to_document() {
            // a includes b, b includes a: a's body used to appear twice because
            // the document itself was never marked as included.
            string tmp_dir;
            try {
                tmp_dir = DirUtils.make_tmp("gdiagram_inccycle_XXXXXX");
            } catch (FileError e) { assert_not_reached(); }
            string a_path = GLib.Path.build_filename(tmp_dir, "a.puml");
            string b_path = GLib.Path.build_filename(tmp_dir, "b.puml");
            string a_body = "@startuml\n!include b.puml\nAlice -> Bob : from_a\n@enduml\n";
            try {
                FileUtils.set_contents(a_path, a_body);
                FileUtils.set_contents(b_path, "!include a.puml\nBob -> Alice : from_b\n");
            } catch (Error e) { assert_not_reached(); }

            var p = new Preprocessor();
            string res = p.process(a_body, a_path);
            FileUtils.unlink(a_path);
            FileUtils.unlink(b_path);
            DirUtils.remove(tmp_dir);

            assert(res.split("Alice -> Bob : from_a").length == 2);
            assert(res.split("Bob -> Alice : from_b").length == 2);
        }

        public static void test_style_translation_keeps_line_numbers() {
            // A one-line <style> block yielding more skinparams than lines (root
            // LineColor gives two, a comma list one per element) shifted every later line.
            var p = new Preprocessor();
            string res = p.process(
                "@startuml\n<style>root { LineColor red }</style>\nAlice -> Bob : hi\n" +
                "<style>class, interface { BackgroundColor blue }</style>\nBob -> Carol : two\n@enduml\n", null);
            string[] out_lines = res.split("\n");
            assert(out_lines[2] == "Alice -> Bob : hi");
            assert(out_lines[4] == "Bob -> Carol : two");
            // The surplus skinparams are still emitted, inside the diagram
            int end = res.index_of("@enduml");
            foreach (var expected in new string[] { "skinparam elementBorderColor red", "skinparam arrowColor red",
                                                     "skinparam classBackgroundColor blue",
                                                     "skinparam interfaceBackgroundColor blue" }) {
                int at = res.index_of(expected);
                assert(at >= 0 && at < end);
            }
        }

        // Index (0-based) of the first output line equal to `marker`, or -1.
        private static int line_index_of(string text, string marker) {
        ...
        // Preprocess `src` and assert `marker` sits at the same line index as in the source.
        private static void assert_marker_keeps_line(string src, string marker) {
            var p = new Preprocessor();
            string res = p.process(src, null);
            int expected = line_index_of(src, marker);
            assert(expected >= 0);
            assert(line_index_of(res, marker) == expected);
        }

        public static void test_inactive_if_keeps_line_numbers() {
            assert_marker_keeps_line(
                "@startuml\n!if 1 == 2\nclass Hidden1\nclass Hidden2\n!endif\nclass Marker\n@enduml\n",
                "class Marker");
        }

        public static void test_else_branch_keeps_line_numbers() {
            // The !else body follows a skipped !if branch
            assert_marker_keeps_line(
                "@startuml\n!if 1 == 2\nclass Hidden1\nclass Hidden2\n!else\nclass Marker\n!endif\n@enduml\n",
                "class Marker");
        }

        public static void test_skipped_elseif_else_keep_line_numbers() {
            // A taken !if skips its !elseif and !else bodies
            assert_marker_keeps_line(
                "@startuml\n!if 1 == 1\nclass Shown\n!elseif 2 == 2\nclass H1\n!else\nclass H2\nclass H3\n!endif\nclass Marker\n@enduml\n",
                "class Marker");
        }

        public static void test_nested_if_keeps_line_numbers() {
            assert_marker_keeps_line(
                "@startuml\n!if 1 == 2\nclass H1\n!if 1 == 1\nclass H2\n!else\nclass H3\n!endif\nclass H4\n!endif\n" +
                "!if 1 == 1\n!if 1 == 2\nclass H5\n!endif\n!endif\nclass Marker\n@enduml\n",
                "class Marker");
        ...
        public static void test_ifdef_ifndef_keeps_line_numbers() {
            assert_marker_keeps_line(
                "@startuml\n!define ON\n!ifdef OFF\nclass H1\n!endif\n!ifndef ON\nclass H2\nclass H3\n!endif\nclass Marker\n@enduml\n",
                "class Marker");
        }

        public static void test_define_and_variable_keep_line_numbers() {
            assert_marker_keeps_line(
                "@startuml\n!define FOO bar\n!define SUM(a, b) a b\n!$x = 3\n!$y ?= \"s\"\n!undef FOO\n" +
        ...
        public static void test_procedure_function_definitions_keep_line_numbers() {
            assert_marker_keeps_line(
                "@startuml\n!procedure $p($a)\nclass $a\n!if $a == \"x\"\nclass X\n!endif\n!endprocedure\n" +
                "!function $f($a)\n!return $a\n!endfunction\n!unquoted procedure U($a)\nclass $a\n!endprocedure\n" +
                "class Marker\n@enduml\n",
                "class Marker");
        }

        public static void test_note_text_gets_no_placeholder_lines() {
            // Placeholders inside a note body would become empty note lines; they
            // follow "end note" instead, so the note text is intact and later lines keep numbering
            var p = new Preprocessor();
            string src = "@startuml\nnote as N1\nline one\n!if 1 == 2\nhidden\n!endif\nline two\nend note\nclass Marker\n@enduml\n";
            string res = p.process(src, null);
            assert(res.contains("line one\nline two\nend note\n"));
            assert(line_index_of(res, "class Marker") == line_index_of(src, "class Marker"));
        }

        public static void test_activity_text_gets_no_placeholder_lines() {
            var p = new Preprocessor();
            string src = "@startuml\nstart\n:first line\n!ifdef OFF\nhidden\n!endif\nsecond line;\nstop\n@enduml\n";
            string res = p.process(src, null);
            assert(res.contains(":first line\nsecond line;\n"));
            assert(line_index_of(res, "stop") == line_index_of(src, "stop"));
        }

        public static void test_quoted_text_gets_no_placeholder_lines() {
            var p = new Preprocessor();
            string src = "@startuml\nusecase U1 as \"first\n!define X 1\nsecond\"\nactor Marker\n@enduml\n";
            string res = p.process(src, null);
            assert(res.contains("\"first\nsecond\"\n"));
            assert(line_index_of(res, "actor Marker") == line_index_of(src, "actor Marker"));
        }

        public static void test_parse_error_line_after_inactive_if() {
            // End to end: the parser's error line is the source line, not one moved up by the skipped branch
            var engine = new DiagramEngine("dot");
            string src = "@startuml\nAlice -> Bob : hi\n!if 1 == 2\nBob -> Carol : a\nBob -> Carol : b\n!endif\nend\n@enduml\n";
            var result = engine.parse(src, null);
            assert(result.diagram_type == DiagramType.SEQUENCE);
            assert(result.errors != null && result.errors.size > 0);
            bool found = false;
...
@enduml""";
        ...
        public static void main(string[] args) {
            ...
            Test.add_func("/preprocessor/include-cycle-back-to-document", test_include_cycle_back_to_document);
            Test.add_func("/preprocessor/style-translation-keeps-line-numbers", test_style_translation_keeps_line_numbers);
            Test.add_func("/preprocessor/lines/inactive-if", test_inactive_if_keeps_line_numbers);
            Test.add_func("/preprocessor/lines/else-branch", test_else_branch_keeps_line_numbers);
            Test.add_func("/preprocessor/lines/skipped-elseif-else", test_skipped_elseif_else_keep_line_numbers);
            Test.add_func("/preprocessor/lines/nested-if",test_nested_if_keeps_line_numbers);
            Test.add_func("/preprocessor/lines/ifdef-ifndef", test_ifdef_ifndef_keeps_line_numbers);
            Test.add_func("/preprocessor/lines/define-variable", test_define_and_variable_keep_line_numbers);
            Test.add_func("/preprocessor/lines/procedure-function-definitions", test_procedure_function_definitions_keep_line_numbers);
            Test.add_func("/preprocessor/lines/note-text-no-placeholders", test_note_text_gets_no_placeholder_lines);
            Test.add_func("/preprocessor/lines/activity-text-no-placeholders", test_activity_text_gets_no_placeholder_lines);
            Test.add_func("/preprocessor/lines/quoted-text-no-placeholders", test_quoted_text_gets_no_placeholder_lines);
            Test.add_func("/preprocessor/lines/parse-error-after-inactive-if",test_parse_error_line_after_inactive_if);
            Test.run();
        }
    }
}
