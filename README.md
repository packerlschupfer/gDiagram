...
![gDiagram editing a Mermaid flowchart with the outline, live preview and properties panel](docs/images/app/main-mermaid-flowchart.png)
...
- **Outline sidebar** — tree of diagram elements (packages, containers, sections) with a filter and collapse that survives re-renders
- **Element palette** — insert snippets for every PlantUML and Mermaid diagram type from the sidebar
- **Properties panel** (`F9`) — edit the clicked element's label, stereotype, colour or id; the source is rewritten as one undo step
- **Search & replace** — in the editor, with match count, case and regex options
...
| Element palette | Outline tree |
|---|---|
| ![Activity diagram with the element palette open](docs/images/app/main-activity.png) | ![Mermaid gantt chart with the outline tree](docs/images/app/main-mermaid-gantt.png) |
...
| Component / Deployment | Components, interfaces, ports on node borders, nodes, artifacts, clouds, databases, nested containers |
